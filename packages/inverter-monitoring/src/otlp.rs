...
//!
//! The same encoding as `packages/system-metrics/src/otlp.rs`, against the same receiver, built
//! on the same `opentelemetry-proto` crate the receiver decodes with. Duplicated rather than
//! shared because these are three standalone crates with no workspace between them -- the
//! convention in this repo -- but the three properties the receiver enforces (non-empty
//! `event_name`, non-zero timestamp, kvlist body) are re-asserted in the tests below so the two
...
pub const SCOPE_NAME: &str = "inverter-monitoring";
...
mod tests {
    ...
    fn sample() -> ExportLogsServiceRequest {
        let records = vec![
            Record::new("inverter.status")
                .with_field("battery_voltage_volts", Value::Double(54.2))
                .with_field("pv2_voltage_volts", Value::Null),
            Record::new("inverter.status.flag")
                .with_attr("flag", Value::str("line_fail"))
                .with_field("asserted", Value::Bool(true)),
        ];
        let resource = vec![
            ("service.name".to_owned(), Value::str("inverter-monitoring")),
    ...
    fn every_record_is_an_event_with_a_timestamp() {
        ...
        assert_eq!(records[0].event_name, "inverter.status");
        assert_eq!(records[1].event_name, "inverter.status.flag");
    }

    /// Where a key sits decides what it is called in a query: the unit's identity has to be a
    /// resource attribute to come back as `resource.attributes.inverter.*`.
    #[test]
    fn identity_is_a_resource_attribute_not_a_record_one() {
        ...
        let resource_keys: Vec<&str> = resource_logs
            ...
            .collect();
        assert_eq!(resource_keys, vec!["service.name", "inverter.serial_number"]);
        ...
        let record_keys: Vec<&str> = resource_logs.scope_logs[0].log_records[1]
            ...
            .map(|kv| kv.key.as_str())
            .collect();
        assert_eq!(record_keys, vec!["flag"]);
    }

    /// A null keeps its key. Dropping it would make an unsupported QPIGS2 look like a schema
    /// change rather than a missing sensor.
    #[test]
    fn a_null_field_is_an_empty_value_not_an_absent_key() {
        ...
        assert_eq!(list.values[0].key, "battery_voltage_volts");
        assert_eq!(
            list.values[0].value.as_ref().unwrap().value,
            Some(any_value::Value::DoubleValue(54.2))
        );
        assert_eq!(list.values[1].key, "pv2_voltage_volts");
