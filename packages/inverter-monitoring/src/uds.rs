...
//!
//! Hand-written rather than hyper + a unix connector: each call sends one request with a known
//! length and reads one response, so an async runtime and a connection pool would be several
//! megabytes of dependency for a code path that is a write and a read.
//!
//! One connection per call, closed by `Connection: close`. This producer is a daemon rather
//! than the oneshot `system-metrics` is, so it makes one of these a minute for the life of the
//! process -- which is still far too rare to be worth keeping a socket open between them.
